...
def insert_chart(input_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    cols = ["chart_id", "chart_date", "chart_open", "chart_high", "chart_low", "chart_close", "chart_volume", "stock_id"]
    ...
    insert_chart_df = pd.DataFrame(rows, columns=cols).sort_values('chart_date').reset_index(drop=True)
    print(insert_chart_df.head())

    return insert_chart_df
...
def select_previous_chart(stock_id: int, chart_date: date) -> pd.DataFrame:
    ...
    sql = text("""
               ...
               """)
    #            여기서 chart_date < :chart_date로 포함 안되도록 함
